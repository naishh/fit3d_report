...
\paragraph{Interactive 3d models}
	...
	impact analysis, driving simulation, military simulation 
	systems.\\
...
related work:
%intro
Lots of research is done on semantical interpretation of urban scenes. We
discuss only specific related work, window detection.
...
They alse matched these details to improve the alignment of the 3d model.
One of the matching models was based on comparison of model edges with the extracted Hough lines.
They disambiguated the lines comparing the angle, location and length difference of the model edge with the extracted Houghlines.
These properties are also used in our window detector.\\
They also detected windows that provided a significant better alignment of the 3d model.
Unfortunatelly the windows where extracted from the holes from laserpoints of a wall, these image where far from accurate.\\
The work of Vosselman provides a usefull practical application of window detection.
And it amplifies the need for a robust window detection technique that doesn't require laserdata.\\
...
In the last decade many different facade classification techniques
are proposed. Pu and Vosselman [11] 
[REFINING BUILDING FACADE MODELS WITH IMAGES, www.isprs.org/proceedings/XXXVIII/3-W4/pub/CMRT09_217.pdf]
fused laser data
and close-range images to reconstruct building facade details.
They extracted windows and doors in both close-range optical
and laser images by using Hough lines. Detected windows
and doors helped them to register close-range optical and laser
images. That shows the importance of the facade classification
study in three-dimensional city modeling.
...
\emph{TODO}
%todo window detection related work
