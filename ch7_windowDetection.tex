...
This projection also comes with the difficulties which we discuss in \ref{}? %
todo
...
\fig{w_Spil1TransCrop1_ImOri.eps}{Rectified image}{0.4}
\fig{w_Spil1TransCrop1_ImEdge.eps}{Result edge detection}{0.4}
\fig{w_Spil1TransCrop1_ImHoughResult.eps}{Houghlines with endpoints}{0.4} 
...
\textit{As the backprojection isn't done at the time of writing we 
...
We first determine the aligment of the windows and then exctract the windows.
...
or the building wall self is occluding a part of the window.  The less frontal
the view, the more occlusion negatively effects the cleaness of the projection.
To handle this occlusion artefact and to increase accuracy we combininging the
windows probabilities.  

\subsubsection{Situation and assumptions}
To be more precise in our assumptoins, we assume the windows have orthogonal sides.  Furthermore we
...
\fig{w_Spil1TransCrop1_ImHibaap.eps}{(smoothed) Histograms and window alignment lines}{0.4}
...
are located at the average positions of the windowedges. Next step is to
...
we can take a look at the distribution of $R$ and $C$. We see two clusters apear: one with
...
we designed another representation in Figure \ref{fig:w_Spil1TransCrop1_ImClassRectGrayscale.eps}
The whiter the area the more probable a rectangle is classified as a window.

\fig{w_Spil1TransCrop1_ImClassRectGrayscale.eps}{TODO}{0.4}
...
\fig{w_Spil1TransCrop1_ImClassRect.eps}{TODO}{0.4}
