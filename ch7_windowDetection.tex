...
\paragraph{Interactive 3d models}
	...
	%todo met bram checken hoe ik png in kan laden
	%\fig{p_simulation_people.png}{1}{Simulation environment}
...
In order to explain our second method of window detection
we need the windows on the facade to be orthogonal and aligned.
Therefore we rectify the facade, this can be achieved in two ways.\\

The first rectification method uses point to point correspondences. This 
requires annotation of the cornerpoints of the facade that are mapped with the
corners of a rectangle.  The downside of this method is that it isn't very accurate.\\
\emph{Isaac/Frans, do you know how I can explain why?}\\
Furthermore the annotation of the cornerpoints is unknown and should be
detected automaticaly with high accuracy.

The second method involves the extraction of a 3D plane of the facade. This 
method is more accurate and sophisticated. It involves a comprehensive process
which is done multiple times by other researchers.
% todo ref fit3d
Therefore we used existing existing software, I. Estebans \emph{FIT3D toolbox},
to extract the 3D model from a series of frames.  And used one of the planes of
the 3D model to rectify the facade, making it ready for robust window
detection.

We calculate the motion between a series of frames in order to extract a
pointcloud of matching features. This pointcloud is used to extract a plane
which is used to rectify the facade by a projection process. We now explain the
steps in more detail.

We began by taking a series of 6 or more consecutive images of a scene.
...
frames.  Not all features will overlap in the frames therefor RANSAC is used to
...
The frames where mathed one by one which returns a motion estimation that is not accurate enough.
Therefore a 3-frame match is done which gives more accurate motion estimation.
This result comes with a certain amount of reprojection error, this error is minimized
using a numerical iterative method called \emph{bundle adjustment}.
The result is a very accurate motion estimation.

The next step is to use this motion data to obtain a set of 3D points
(corresponding to the selected image features).  For this a linear triangulation
method is used. 

Next a RANSAC driven planefitter is used to accuratly fit a plane through
the 3D points. 
%todo figure

\paragraph{Efficient Projecting} 
Now we extracted the 3D plane of the facade, the next challenge is to rectify the facade. 

It would be straight forward to rectify the full image. However this is computational
very expensive as each pixel needs to be projected and rotated. To keep the
computational cost to a minimum we project only the Houghlines endpoints. 
This is allowed because the projective transformation we apply preserves the straightness of the lines.

This leaves the edge detection and Houghline extraction to be applied on the 
unrectified image. 
...
this means 600 projections in stead of 1572864: a factor of almost 3k faster.\\

%This is done by performing a projection and rotation.

The Houghline endpoints are projected to the 3D plane we extracted.
This is done on the same way as the skyline detection, by sending rays from the camera center
trough the houghline endpoints and calculating the intersection with the 3D plane.
The result is a 3D point cloud where each point is labeled to there corresponding Houghline.
I
The next step is to rotate the camera in a way that the facade (and the Houghlines) are seen
upfront. This means the viewing direction (z-axis) of the camera needs
...
We calculated a rotation matrix using the axis-angle representation.
...
two vectors.
The magnitude of the rotation $\alpha$ is equivalent to the angle $z$ between $z'$ given $u$. $\alpha$ and $u$ are used to create a
rotation matrix $R$ which is aplied to the 3D point cloud.
The result is a set of rectified houghlines.
