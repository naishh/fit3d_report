...
There are two methods to facade rectifation.
i---------------------
Using a point to point correspondence.
click 4 Corners 
%however this method is bad, drugs are badddddd
\emph{why?}

%intro
We use the \emph{fit3d toolbox} to extract a 3d model from a series of frames.
The 3d model is used to rectify the facade.

We briefly explain the methods that are used. 
%todo rehearse shortly the motivation
steps
% todo ref fit3d


	\paragraph{Setting up the dataset}
	...
	\paragraph{Motion estimation}
		...
		After this an 8-point algorithm together with a voting mechanism is used to extract the camera motion.

	\paragraph{Motion refinement}
	...
	\paragraph{3D point cloud}
		The next step is to use this motion data to obtain a set of 3D points (corresponding to the selected image features).
		For this a linear triangulation method is used. 

	\paragraph{Plane fiting}
	%todo
		walls
