...
section is about two developed methods for robust window detection.
...
using approaches from a different point of view.
...
To summarize, the work of Pu and Vosselman provides a useful practical application of window detection and it amplifies the need for a robust window detection technique that is independed of laser data.\\
...
computational expensive procedure.  Therefor we do not consider the refinement
...
describe the windows with Haar like features which are fet into a (Ada boost) cascaded decision tree.
...
Te retrieve the actual windows, each connected corners is mirrored on its
