...
segment is classified as the most likely wall for that line segment.
	...
	%--------------------------------------------------------------------------------------
	%TODO Better changing, and think of a right place in the chapter
	every wall of the building exist of four coordinates
	wall spans plane
	point to 3d line
	line intersects plane
	for everywall intersection is found unless line is parallel
...
and therefor also present a 3D point in space.
...
What is known, is de 2D information of the pixel, this is the location of the
pixel in the image. We know what camera took the picture so we can calculate a
camera specific Calibration matrix. This reduces the possible points in 3D space
to an infinite line.  

This line is defined by two coordinates:
coordinates:\\ 
\begin{itemize}
	\item $C$, the center of the camera 
...
%TODO explain in detail what K is and how it is retrieved, maybe write this down in preliminaries?
\end{itemize}
%TODO image
...
A system is calibrated if the camera centers and camera viewing directions are
known. In some systems this is recorded on forehand. In our system we retrieved
this values by using the fit3d toolbox.\\  %TODO REF
%TODO uitleggen hoe (fit3d toolbox)
$K'p$ is the point that lies on the retina of the camera 

The infinite line of possible points in 3D is defined by the coordinates $C$ and $K'p$
%TODO describe how a infinite line is described by 2 formulas


As described in %TODO
 the 3D building model is divided into different walls.  
...
Because the 3D model is an estimate the walls maybe just
to short. Thats why we let every wall span an infinite plane.

%TODO formula

 We have a 3d model
 This helps us to find where the line segment 
 
Intersections are then calculated between the lines and the planes spanned by the building walls.\\
