...
This method like a very sophisticated one, as it is accurate and autonomous.
...
\fig{outputSkylineSpil-Im13-thresh030.eps}{The output of the skyline detector on the \emph{Folkert} dataset: a scene which violates the top sharp edge assumtion. The hanging streetlight causes the detection of a sharp edge above the building. This results in a damaged skyline.}{0.3}
\clearpage
...
of this project.  Some ideas for future research are shared next.

\subsection{Future research}
\label{sec:skylinefut}
\subsubsection{Automatic thresholding}
As the threshold that decides wheter an edge is strong enough to represent the
...
dataset are highlited. For the highlited position the skyline happens to be the
