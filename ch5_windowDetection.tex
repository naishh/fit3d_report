...
Edge detection is done as it is described in section 
\ref{sec:edge}
We extract two groups of Houghlines which fall in the two window directions 
...
We use ranges because 1)
the user hardly ever holds the camera exactly orthogonal.
2) we work with unrectified facades, meaning we deal with perspective distortion.
To be more concrete, if the user takes a photo (Figure \ref{fig:cameraPitch.eps} with a certain Yaw \= 0, the horizontal lines become skew.
The range of the vertical group is smaller as the user 
often takes photo with a low pitch
