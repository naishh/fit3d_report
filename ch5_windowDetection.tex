...
\TODO{ the windows of this dataset differ in size, type, shape and are partially unaligned}
...
\subparagraph{Many alignment lines}
...
\subparagraph{Fusing the methods}
...
\TODO{Explain datasets (before results?):}
%,  The Spil dataset is perspective transformation,
%occlusion bla bla. The Dirk dataset is a more challenging dataset as the windows are very close and differ in type}
%ataset: Dirk, Rectified image of a realistic scene which is realistic as it contains light spots, bicycles. Note that the windows are partially aligned and the differ in size and type}{0.45}
...
\TODO{image of concave and convex shape of Dirk dataset, }%w_Spil1TransImproved_WAlinesMerged.eps
\TODO{image of original dirk dataset }
\newpage
\fig{w_Dirk4Trans_ImOri.eps}{Dataset: Dirk, Rectified image of a realistic scene which is realistic as it contains light spots, bicycles. Note that the windows are partially aligned and the differ in size and type}{0.45}
...
\begin{table}[t]
\caption{Window detection results on Anne2 and Dirk dataset}
\begin{tabular}{|l||c|c|}
\hline
...
\TODO{The grouping rates are calculated as follows, }
...
\TODO{say something about grouping  error and future work: not only adjacent
but also a min width between gaps}
...
\subparagraph{Interpretation of the results of the Anne dataset}
\TODO{Meer over lof spreken} 
\TODO{..}
\TODO{1st dataset so its overfitted?}
