...
From the introduction we learned that window detection can play an importand role in
...
This chapter is about our developed methods of window detection it is organised
as follows:\\
  We start related work and put our work in context.  Next we describe our first
  window detection approach that is invariant to viewing direction.  After this
  we present a facade rectification method. Next this method is used for our
...
promissing approach to window detection because it has to be supervised.  This means
...
windows. Unfortunatelly due to bad laser range data, the results where far from
accurate.\\
...
Edge detection is done using the Canny edge detector motivated earlyer in
...
\TODO2{maak analogie met L en H patronen hoe we die herkennen in de hersenen}
...
even complete rectangular shapes.  The later is difficult because the edges are
...
We could als incorporate not only the center of the connected corner as a
...
corners of a rectangle. This mapping is used to calculate a transformation matrix. 
 The downside of this method is that it isn't very accurate as it doesn't take
...
are using Houghlines we project only the coordinates of the endpoints of the found Houghlines. 
...
way as we explained in chapter \ref{chap:skylinedetection}. We have send
...
The next step is to transform the facade (and therefore the Houghlines) to a
...
The result is a set of rectified 3D points that are grouped to their Houghlines.
...
\TODO2{tabel van datasets en handicaps!}
...
\TODO2{peak merging result misses for 1 dataset?}
\TODO2{peak merging result plaatje met en zonder merging onder elkaar}
...
convex shapes at nonwindow locations}{0.45}
