...
% SECUNDAIRE todos:
% classificatie methode met probs en met derivitave met elkaar vergelijken
% iets zeggen over complexiteit
...
approaches that differ of our approach.  After this, we discuss state of the art
...
to assume repeating and aligned windows.  Furthermore they matched template window
objects which they predefine, this constraints the variety of windows that could
be matched.  At last they use expensive algorithms that make it impossible for the system
to run in real time.  \\
...
they search and group intersecting L-shapes to close a window. A shape is
determined closed if it can seperate an inside region from the outside.
%todo afflammen

Ali et all \cite{Ali_facades} describe the windows with \emph{Haar} like
features, the featerus are combined using a cascading classifier. The cascading
classifier which can be seen as a decision tree is learned using the \emph{Ada
boost} algorithm.  They also use the window detection to determine the region of
the facade.
...
annotated.  Furthermore the cascader is always overfitted to the learning data
and is sensitive to scale, making it hard to generalise (detect windows that are
not included in the dataset).  A more general descriptor of the window is
desirable.\\
...
was to have low requirements on the input data.  Our system doesn't need need
...
next).
In order to project edges they rectify the facade. 
The edges are projected into their orthogonal direction, a function is developed that counts the amount of projected edges the 'Projection profile', Figure \ref{fig:p_projection_profiles.eps}.
\fig{p_projection_profiles.eps}{Projection profiles of Lee and Nevatias work}{0.4}
A threshold is applied to indicate the window boundaries and alignment.
For example they use a vertical edge projection to establish the horizontal division of the windows.\\
The next step, they labeling the areas containing windows and use color to disambiguate the windows.
...
Furthermore the classification based on color is sensitive to the change in lightning conditions.
...
shapes of windows can exist in the same column, they only use the window alignment as
...
computational expensive procedure and therefore not suitable for our application. \\

\subsubsection{Our work in context}
The window alignment procedure in \cite{Recky_kmeans} and \cite{Lee_extraction}
is very robust.  Therefor we decided to use this method as a basis and improved
the alignment algorithm furthermore we build a different window classification method.  

For both window alignment and classification we use the same source, edge information.
This means we don't require color transformation and only apply transformation on line
segment endpoints. This makes our algorithm invariant to lightning conditions and
perform in real-time.  
	
	In the previous work \cite{Recky_kmeans} and \cite{Lee_extraction} they use
	a single projection profile for each direction.  We improved the window
	alignment process by fusing two projection profiles for each direction.
	E.g. for the determination of the window row alignment we fuse both
	horizontal and vertical projection profiles.  Furthermore we build
	a alternative window classification procedure which is based on a higher
	level of shape interpretation of these projection profiles.
