...
A window alignment position is either found by one method or by both methods.

If the window alignment is found by one method we cannot back it up with the other method, therefor 
we increase the threshold from 0.5*max peak to 0.7*max peak to make it more certain and 
The target is to have as little as possible false positives but don't mis alignment locations.

Otherwise, if the window alignment position is found by both methods, often the peaks are located very close 
to eachother. How do we know 
Sometimes the peaks are shifted 
%--------------------------------------------------------------------------------------------------------------------------------
...
\fig{w_Dirk4Trans_ImOri.eps}{Dataset:Dirk, original image} 
\fig{w_Dirk4Trans_ImHibaap_Xvh.eps}{Dataset:Dirk, window alignment lines}
\fig{w_Dirk4Trans_ImClassRect.eps}Dataset:Dirk, window alignment lines}
