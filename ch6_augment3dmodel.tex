...
\subsubsection{Hough transform}
	...
	If a pixel is classified as a skyline pixel, the Hough transform increases a
	vote value for every valid line ($m$,$b$ pair) that crosses this particular
	pixel.  Thus lines ($m$, $b$ pairs) that receive a large amount of votes
	contain a large amount of skyline pixels.  Because it finds straight lines
	with skyline pixels it is most likely that it returns parts of the building
	contour. 
	Optional the Hough transform returns the start- and endpoint of the lines 
	which is in our case useful as we look for specific building contour parts. 
