...
% 	%TODO example image
	...
	%This fact is used as a Heuristic assumption to guide the search for the best %tits, hahahahah
	see figure%TODO.
	%This fact is used as a heuristic to search for the most probable wall.

	Every line segments is projected to all walls and the overlap is measured. 
	The wall with the largest overlap is classified as the most probable wall.
	The algorithm returns a set of lines in $R3$, which represent the Hough line segments projected to their most probable wall.

	How the overlap is calculated is now explained in detail.
	

	$l_{proj_w} can overlap $w$ in four different scenarios:
		1) no overlap (fig %TODO)
		2) partial overlap (fig %TODO)
		3) full overlap (includes)(fig %TODO)
		3) full overlap (overextends) (fig %TODO)
