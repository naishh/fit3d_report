...
\subsubsection{Augmenting the 3D model} %todo other word then augment?
	...
	% TODO image
	% TODO footnote: FOODNOTE:Note that this method assumes that the walls are alligned on the y-axis, which is in our dataset the case.
...
Image %TODO
shows the top 3 longest Houghlines, the endpoints are denoted as a black and
blue cross. All three lines lie on the building contour.  The left line covers
only a part of the buildingwall. The left and middle line are connected. The
right line covers the wall untill the tree occludes.\\

Image %TODO
displays the line segments (originated from different views) projected onto their associated walls.
For a clear view we only took three walls of the building.
...
In image %TODO
the midpoints of the projected linesegments are collected averaged. This
point is used as the new height of the building. \

Figure %TODO
displays the augmented 3D model. The cornerpoints of the walls are adjusted according the calculated wall heights. The left and middle wall are extended and the right wall is shortened.
...
give a good example of the corner prolem. Both endpoints could easily be associated with the wrong wall (even if the rough 3D model is very accurate). Fortunatelly we use the middle point of the line segment to determine the wall. And as can be seen in figure 
%TODO
this is always the correct wall.
%TODO
...
Consider image, %TODO
the roof consisting of two planes which are not parallel with the facade of the building. This makes the problem of extracting the 3D model more complex but not 
