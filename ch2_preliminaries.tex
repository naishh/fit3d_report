...
Figure \ref{fig:coordinateSystems.eps} (x,y) span this frame.
...
scene and the position of the cameras in the world in (ijk). A overview can be
seen in Table \ref{tab:coord}.
...
the relationship between what appears on the image (or camera retina) and where
 ...
 This involves calculating the Intrinsic and extrinsic parameters.
...
the \emph{Anne} dataset we calculated K with the Bouguet toolbox
\cite{bouguet}.
This method involves making images of a chessboard in
different positions and orientations and is explained next
[DRAFTnote: not finished at the
time of writing].

\subsubsection{Determine the intrinsic camera parametrs with the Bouguet toolbox}
\label{sec:bouguet}
Todo
%  TODO
...
and the viewing directions. These are unique for every view.
%todo explain rotation, translation
In some systems these are recorded by measuring the cameras position and
headings at the scene. In other systems this is computed afterwards from the images.  
...
\fig{fit3d_planes.eps}{Fitted planes}{0.4}
