...
autonomous. Because we have a stable scene with sharp edges at the
building contour this method would be an overkill.  % TODO images?


\subsubsection{Horizon detection for Unmanned Air Vehicles}
%TODO images?
In this domain, the horizon detector for Unmanned Air Vehicles takes can take advantage of the high
altitude of the vehicle and therefor the horizon can be approximated to be a
straight line. 
In [], %TODO Ref vision-guided flight stability and control for micro air vehicles
an algorithm is proposed which is based on the idea that this straight line separates the image
onto two regions that have a different appearance. They use color as a measure
of appearance and generate two color distributions: one for the sky and one
for the ground. The distributions are optimized using bisection search on
	a criterion that uses the covariance and the eigen values of the
	distributions. The line that best separates the two distributions is
	determined to be the horizon.\\
...
%TODO better chain? merge this alineas?
As mentioned in the introduction, in this project we use the skyline to
...
From the related work the %TODO REF Planetary Rover localization [9] 
...
% (column wise algo part of the skyline detector )
